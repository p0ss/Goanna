...
coarse volume. What comes back per mapblock is 92 bytes, protocol version 7:
flags for available and complete data, a 4 by 4 by 4 field of solid content
indices, packed liquid surface heights, the block's maximum day and night
light, one liquid content index, and a 64-cell liquid mask. Solid and liquid
occupancy are independent, so shallow water retains both its opaque seabed
and its exterior water surface. Index 0 is known air and 255 is unknown, so
vertical occupancy is retained: a
...
an 8 by 8 by 8 block area, the size a request covers, is about 47 KB raw and
63 KB once base64 encoded for the channel.
