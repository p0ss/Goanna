...
OUT=ROOT/('build/dig-gif' if options.gif else 'build/dig-boundary')
...
def cmd(name,**args):
    ...
    return result
server=launch(['flatpak','run','--die-with-parent',f'--filesystem={SCRATCH}','--command=luanti','org.luanti.luanti','--server','--world',str(world),'--gameid','minetest','--config',str(config)],'server.log')
client=None
try:
    time.sleep(4)
    env=os.environ.copy()
    env.update(GOANNA_HOST='127.0.0.1',GOANNA_PORT='30579',GOANNA_NAME='digboundary',GOANNA_CONTROL='30879',GOANNA_VIEW_RANGE='5',GOANNA_TOD='0.5',GOANNA_BODY='0',GOANNA_CARVE='0.12',GOANNA_NO_PBR='1',GOANNA_SDFGI='0',GOANNA_AMBIENT='2',GOANNA_BEVEL='0',XDG_DATA_HOME=str(SCRATCH/'profile'),XDG_CONFIG_HOME=str(SCRATCH/'config'))
    ...
    if options.gif:
        ...
        cmd('quit')
    for label,base in ([] if options.gif else [('interior',7),('boundary',15)]):
