"""Hand authored LabPBR height and smoothness for mcl_deepslate_redstone_ore.

The 16 px art's grey texels are mcl_deepslate's own palette pixel for
pixel, zero distance from one of its nine colours. Sixty seven texels sit
0.13 to 0.42 further out, a clean gap above the matrix's zero, with a red
cast (r above g by 0.25 to 0.59, against the matrix's own 0.00 to 0.01)
that no crack or bedding face reaches. Redstone here is one large clump
again, the same flat, glittery facets as the core stem's.
"""
import sys
...
STEM = "mcl_deepslate_redstone_ore"
...
def main():
    ...
    ore_labels_hi = lib.warp_labels(ore_labels, amp=8.0, seed=171)
    ...
    raw = lib.fbm(lib.SIZE, base_cells=8, octaves=2, seed=172, gain=0.5)
    levels = 3.0
    facet = np.round(raw * levels) / levels * 0.03
    glitter = lib.white_noise(lib.SIZE, seed=173) * 0.015
    ore_height = 0.84 + facet + glitter
    ...
    sparkle = np.clip(lib.white_noise(lib.SIZE, seed=174), 0.55, 1.0) - 0.55
    smooth = 0.6 * height + 0.45 * rough_noise + ore_t * 0.28 + ore_t * sparkle * 0.9
