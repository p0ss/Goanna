"""Hand authored height and smoothness for kythen_khmer_areca.

The 32 px art is a cut-out, 16 percent alpha punched out in 56 small
patches (one to fifteen texels), the finest and most numerous of the four
foliage cut-outs in this batch: sky showing through many small gaps rather
than a few large ones. An areca is a feather palm, its fronds made of many
narrow leaflets, which matches this finer, denser hole pattern better than
a single broad leaf would. Built with the same lobe scatter as
kythen_khmer_kapok.py but at a smaller radius and higher count, so the
individual "leaflets" read smaller and more numerous, the areca's fine
feathered look against kapok's broader canopy.
"""
...
def main():
    ...
    print(f"alpha: {(alpha16 < 0.5).sum()} of {alpha16.size} texels transparent "
          f"({100.0 * (alpha16 < 0.5).mean():.0f}%), fine scattered gaps: a feathered frond, not a solid trunk")
    print("class:", CLS)
    ...
    lobes_fine = scatter_lobes(SIZE, guide, 650, seed=1201, radius_range=(2, 4))
    lobes_coarse = scatter_lobes(SIZE, guide, 250, seed=1202, radius_range=(5, 8))
    lobes = np.maximum(lobes_fine, 0.8 * lobes_coarse)
    lobes = lib.normalise01(lobes)

    grain = lib.fbm(SIZE, base_cells=42, octaves=2, seed=1203, gain=0.5)
