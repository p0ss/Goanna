...
def main():
    ...
    # bevel carrying almost all of the real shape, not a pitted surface.
    # Denser grain than the old, unclipped version (130 cells rather than
    # 44): a face this flat still needs to tile without a visible step at
    # the wrap join, and finer grain gives the interior of the tile its own
    # small texel to texel variation to measure that step against, without
    # needing a bigger height amplitude to get there.
    crystal = lib.fbm(lib.SIZE, base_cells=130, octaves=3, seed=62, gain=0.55)
    pores = lib.blur(lib.white_noise(lib.SIZE, seed=63), 1)
