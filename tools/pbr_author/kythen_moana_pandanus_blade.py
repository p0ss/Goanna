"""Hand authored height and smoothness for kythen_moana_pandanus_blade.

The 32 px art is a cut-out, 86 percent opaque, the same mottled dither
against transparency as kythen_moana_palm_frond.py with no drawn vein or
outline. Built the same way: a scatter of long blade ridges, mostly
upright with a little lean, tapering near the tip, a raised midrib down
the centre, held inside an envelope taken from the alpha shape's own
warped distance to edge so the height still falls to nothing at the cut
boundary. Pandanus blades are narrower and stiffer than a palm frond's, so
the blade widths here are pulled in from the frond script's own range.
"""
...
GAME = "kythen"
STEM = "kythen_moana_pandanus_blade"
...
def blade_stamp(radius, angle, length, width, amp, midrib_amp=0.30, tip_frac=0.35):
...
def main():
    ...
    blades_fine = scatter_blades(SIZE, guide, inside, 45, seed=SEED,
            length_range=(60, 110), width_range=(6, 9))
    blades_coarse = scatter_blades(SIZE, guide, inside, 14, seed=SEED + 31,
            length_range=(110, 170), width_range=(8, 12))
