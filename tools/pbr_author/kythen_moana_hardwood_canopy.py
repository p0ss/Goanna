"""Hand authored height and smoothness for kythen_moana_hardwood_canopy.

The 32 px art is 90 percent opaque, a per texel dither, lum 0.323 to
0.509, the same face on overlapping lobe canopy as
kythen_moana_breadfruit_canopy.py and kythen_moana_candlenut.py, read and
built the same way: small domes for lobes, taller where the art is
lighter, a shallow rim drop and a faint midrib on each, the alpha holes
carved to the deepest point through the whole canopy.
"""
...
GAME = "kythen"
STEM = "kythen_moana_hardwood_canopy"
...
def main():
    ...
    lobes_fine = scatter_lobes(SIZE, guide, 700, seed=1401, radius_range=(6, 9))
    lobes_coarse = scatter_lobes(SIZE, guide, 260, seed=1402, radius_range=(10, 15))
    lobes = np.maximum(lobes_fine, 0.8 * lobes_coarse)
    lobes = lib.normalise01(lobes)

    grain = lib.fbm(SIZE, base_cells=40, octaves=2, seed=1403, gain=0.5)
