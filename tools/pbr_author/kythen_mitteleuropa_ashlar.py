"""Hand authored LabPBR height and smoothness for kythen_mitteleuropa_ashlar.

The 32 px art carries three grey shades. The brightest, 0.786, appears
nowhere except four full width rows (0, 8, 16, 24), a clean running bond of
four courses, each seven texels of dressed stone plus one texel of mortar.
Each course also has exactly two full height mortar columns, sixteen texels
apart, staggered by four texels from the course above: 0 and 16 in the
first course, 12 and 28 in the second, 8 and 24 in the third, 4 and 20 in
the fourth, the same staggering kythen_habesha_aksumite_ashlar.py's art
draws. Every course already carries two joints, so flood fill on the non
mortar texels separates the two blocks in every course cleanly with no
wrap ambiguity. This is dressed ashlar: flat faces, fine continuous joints,
no domed rubble, the same reading as the Habesha stem.
"""
import sys
...
GAME = "kythen"
STEM = "kythen_mitteleuropa_ashlar"
...
def label_blocks(mortar_mask):
    ...
    the wrap never merges two blocks into one; no seam cut is needed."""
...
def main():
    ...
    wobble = lib.fbm(SIZE, base_cells=32, octaves=2, seed=811) * 0.8
    ...
    crown = lib.fbm(SIZE, base_cells=5, octaves=2, seed=812) * 0.04
    layout = layout + crown * t
    tooling = lib.fbm(SIZE, base_cells=52, octaves=3, seed=813, gain=0.55) * 0.05
    pores = lib.blur(lib.white_noise(SIZE, seed=814), 1) * 0.04
    # One genuinely deep pore per block face, the odd chip a dressed block
    # gets over the years: a real narrow pit rather than the joint's broad
    # even groove.
    pit_field = lib.blur(lib.white_noise(SIZE, seed=815), 1)
    ...
    # variation on top.
    rough_noise = lib.fbm(SIZE, base_cells=20, octaves=3, seed=816)
