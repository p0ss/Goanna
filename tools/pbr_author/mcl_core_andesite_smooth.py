"""Hand authored LabPBR height and smoothness for mcl_core_andesite_smooth.

The 16 px art is a single polished slab, the same construction as
mcl_core_granite_smooth and mcl_core_diorite_smooth: seven shades from
0.338 to 0.549, lib.segments finds only faint crystal flecks in one
connected body. The edge carries the bevel again: row 0 (mean 0.514) and
column 1 (0.518, with column 0 a milder 0.486) sit above the overall mean
(0.439), row 15 (0.358) and columns 14 and 15 (0.362, 0.358) sit below it,
...
def main():
    ...
    labels_hi = lib.warp_labels(label, amp=2.5, cells=10, seed=81)
