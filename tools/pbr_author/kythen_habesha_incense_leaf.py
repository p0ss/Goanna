"""Hand authored height and smoothness for kythen_habesha_incense_leaf.

The 32 px art is a canopy seen face on, a dithered mix of green and tan
texels with small clusters punched to alpha zero for sky between the
foliage: the same kind of layout as mcl_core_leaves_big_oak, but busier,
with more colour variation texel to texel. Boswellia (incense trees) carry
small pinnate leaflets, fine and feathery, so the relief uses many small
lobes, smaller and more numerous than the oak canopy's, giving a finer,
busier texture with no single leaflet standing out the way a fig leaf's
lobes do. The alpha holes are gaps through the whole canopy and are
carved to a low floor regardless of what lobe sits there.
"""
...
def lobe_stamp(radius, angle, amp, rim_frac=0.78, rim_depth=0.10,
        midrib_amp=0.16, midrib_sigma_frac=0.22):
    """A gentle dome, a faint midrib and a shallow rim groove, sized for
    a small feathery leaflet rather than a broad leaf."""
...
def build(out_dir):
    stem = "kythen_habesha_incense_leaf"
    ...
    # Many small lobes, smaller radius range and a higher count than the
    # oak canopy: fine pinnate leaflets, not broad undifferentiated mass.
    lobes_fine = scatter_lobes(SIZE, guide, 1100, seed=6201, radius_range=(3, 5))
    lobes_coarse = scatter_lobes(SIZE, guide, 380, seed=6202, radius_range=(6, 8))
    lobes = np.maximum(lobes_fine, 0.75 * lobes_coarse)
    lobes = lib.normalise01(lobes)

    # Fine cuticle grain, busier than the oak canopy to match the art's
    # own texel to texel colour variation.
    grain = lib.fbm(SIZE, base_cells=48, octaves=2, seed=6203, gain=0.5)

    canopy = lib.normalise01(0.80 * lobes + 0.20 * guide + 0.10 * (grain * 0.5 + 0.5))

    hole_floor = 0.03
    height = canopy * alpha_soft + hole_floor * (1.0 - alpha_soft)
    height = np.clip(height, 0.0, 1.0)

    variation = lib.fbm(SIZE, base_cells=22, octaves=3, seed=6204, gain=0.55)
