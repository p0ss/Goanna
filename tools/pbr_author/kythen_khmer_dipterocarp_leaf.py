"""Hand authored height and smoothness for kythen_khmer_dipterocarp_leaf.

The 32 px art is a cut-out, 9 percent alpha punched out in 35 small patches
(one to twelve texels), the same scattered small hole pattern
mcl_core_leaves_big_oak.py's own art has. Built the same way, lobed domes
scattered two scales deep over the art's own brightness, holes carved to a
shared dark floor; the alpha channel is kept in the albedo, since this is a
cut-out drawn through the scissor shader, not a solid face.
"""
...
GAME = "kythen"
STEM = "kythen_khmer_dipterocarp_leaf"
...
def main():
    ...
    print(f"alpha: {(alpha16 < 0.5).sum()} of {alpha16.size} texels transparent "
          f"({100.0 * (alpha16 < 0.5).mean():.0f}%), scattered lobe gaps")
    print("class:", CLS)
    ...
    lobes_fine = scatter_lobes(SIZE, guide, 600, seed=1301, radius_range=(3, 5))
    lobes_coarse = scatter_lobes(SIZE, guide, 230, seed=1302, radius_range=(6, 9))
    lobes = np.maximum(lobes_fine, 0.8 * lobes_coarse)
    lobes = lib.normalise01(lobes)

    grain = lib.fbm(SIZE, base_cells=42, octaves=2, seed=1303, gain=0.5)
