"""Hand authored LabPBR height and smoothness for mcl_deepslate_iron_ore.
...
0.08 to 0.55 further out, a clean gap above the matrix's zero, with a warm
rust cast (r above b by 0.07 to 0.22, against the matrix's own 0.00 to
0.02) that the deepslate's own near neutral grey never reaches. That is
the iron, rounded and rough exactly as the core stem's is.
"""
import sys
...
def main():
    ...
    ore_labels_hi = lib.warp_labels(ore_labels, amp=8.0, seed=141)
    ...
    bump = lib.fbm(lib.SIZE, base_cells=14, octaves=2, seed=142, gain=0.5) * 0.05
    grit = lib.blur(lib.white_noise(lib.SIZE, seed=143), 1) * 0.09
    ore_height = 0.85 + bump + grit

    height = matrix_height * (1 - ore_t) + ore_height * ore_t
    height = lib.normalise01(height, 0.5, 99.5)
    print(f"height sd {height.std():.3f}")

    # Native iron is a metal: the nugget gets metal_mask, so the shader
    ...
    ore_smooth = 0.93 + bump * 0.2 - grit * 0.3
