"""Hand authored height and smoothness for mcl_core_log_spruce_top.

Same edge-ring, sawn-face split as default_tree_top.py and
mcl_core_log_big_oak_top.py; see default_tree_top.py's docstring for the
reasoning. This texture's ring runs 0.137 to 0.194, the face 0.162 to
0.455: a real gap, similar in size to default_tree_top's.

Spruce's own bark on the side (mcl_core_log_spruce.py) is scaly, cut into
many narrow plates rather than a few deep furrows, and this top follows
that lead in its own way: the ring to face groove is the narrowest of the
four species (distance_to_edge capped at 2 texels) and the deepest
(0.85 to 0.30), which is what it took to clear the 0.35 jointed-surface
ao_min target here, the same shape of fix mcl_core_log_big_oak_top.py
needed and for the same reason: a species whose bark, on the side, reads as
rougher than default_tree's needs a correspondingly sharper cut on the top
to read as the same log.
"""
...
STEM = "mcl_core_log_spruce_top"
CLS = lib.class_of(STEM)
SIZE = lib.SIZE
SEED = 301  # shared with mcl_core_log_spruce.py, the side of the same log
...
def main():
    ...
    ring_max_dist = 2  # narrow and deep; see module docstring
    ...
    target_ring, target_face = 0.85, 0.30
    ...
    ring_grain = blur_axis(ring_grain_src, radius=10, axis=0) * 0.14
