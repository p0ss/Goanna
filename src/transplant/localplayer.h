...
public:
	...
	LocalPlayer(IGameDef *gamedef, const std::string &name);

	// Set by the session from the server's granted privileges.
	struct Privileges {
		bool fly = false;
		bool fast = false;
		bool noclip = false;
	} privileges;
	// Player object step height (server default 0.6 * BS for players).
	float step_height = 0.6f * BS;
	virtual ~LocalPlayer();
	...
	void applyControl(float dtime, Map *map);
