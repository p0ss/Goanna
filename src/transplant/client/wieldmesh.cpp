...
// is Goanna's stand-in. setItem, setExtruded, setColor, the extrusion mesh
// cache, createGenericNodeMesh, createAnimationFrames, createItemMesh and
// getExtrudedMesh are otherwise verbatim.
...
{
	// Goanna: filtering is decided by the Godot materials
	m_anisotropic_filter = false;
	m_bilinear_filter = false;
	m_trilinear_filter = false;

	// If this is the first wield mesh, create a cache
...
void WieldMesh::setExtruded(const TileDef &d0, const TileLayer &l0,
...
void WieldMesh::setExtruded(video::ITexture *texture,
		video::ITexture *overlay_texture, v3f wield_scale)
{
	...
	m_scale = wield_scale * WIELD_SCALE_FACTOR_EXTRUDED;
...
void WieldMesh::setItem(const ItemStack &item, Client *client, bool check_wield_image)
{
	...
	if (def.type == ITEM_NODE) {
		...
		default: {
			...
			changeToMesh(mesh);
			mesh->drop();
			m_scale = wield_scale * WIELD_SCALE_FACTOR
				/ (BS * f.visual_scale);
			break;
		}
		}

		u32 material_count = m_mesh ? m_mesh->getMeshBufferCount() : 0;
		for (u32 i = 0; i < material_count; ++i) {
			video::SMaterial &material = m_mesh->getMeshBuffer(i)->getMaterial();
...
void WieldMesh::setColor(video::SColor c)
{
	scene::IMesh *mesh = m_mesh.get();
	if (!mesh)
		return;
...
void WieldMesh::setLightColorAndAnimation(video::SColor color, float animation_time)
{
	if (!m_mesh)
		return;
...
void WieldMesh::changeToMesh(scene::IMesh *mesh)
{
	// Goanna: no dummy cube; a null mesh means nothing to draw
	if (mesh) {
		mesh->grab();
