// Transplanted from luanti/src/collision.cpp (LGPL-2.1-or-later).
// Goanna changes: takes a Map* instead of an Environment*; object collision
// (which needed the client/server environments) is stubbed until Goanna has
// its own active objects; profiler names are fixed strings.
// Luanti
...
static void add_object_boxes(Map *map,
		const aabb3f &box_0, f32 dtime,
		const v3f pos_f, const v3f speed_f, ActiveObject *self,
		std::vector<NearbyCollisionInfo> &cinfo)
{
	// Goanna: object collision not yet available (no active objects yet).
	(void)map; (void)box_0; (void)dtime; (void)pos_f; (void)speed_f; (void)self; (void)cinfo;
}

#define PROFILER_NAME(text) ("Goanna: " text)
		...
		StepUpMode step_up_mode)
{
	...
	*/
	...
	if (collide_with_objects) {
		add_object_boxes(map, box_0, dtime, *pos_f, aspeed_f, self, cinfo);
	}
...
bool collision_check_intersection(Map *map, IGameDef *gamedef,
		...
		bool collide_with_objects)
{
	...
	if (collide_with_objects) {
		v3f speed;
		add_object_boxes(map, box_0, 0, pos_f, speed, self, cinfo);
	}
