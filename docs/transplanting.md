...
| `src/transplant/client/mapblock_mesh.cpp` | `src/client/mapblock_mesh.cpp` | Uses the Irrlicht video driver | No hardware mapping hint, no minimap blocks; `PartialMeshBuffer::draw()` is a no-op; defines `g_goanna_no_light`, which makes `encode_light()` return opaque white and so disables the baked vertex lighting; transparent buffers keep their indices; tile light-source ownership is copied into the high bit of each vertex's `Aux`; a crack tile carrying `GOANNA_PERSISTENT_CRACK` bakes its stage directly instead of registering into `m_crack_materials`, so `animate()`'s one deferred level (the live dig only) cannot overwrite a stored carve's own stage; otherwise verbatim |
| `src/transplant/client/content_mapblock.cpp` | `src/client/content_mapblock.cpp` | Reaches `Client` | Compiles against Goanna's `Client` stand-in; `applyFacesShading` calls are skipped while `g_goanna_no_light` is set; `drawBeveledSolid` added, and `drawSolidNode` routes bevelled nodes to it while `g_goanna_bevel` is above zero; `isFakeLiquid` added, a liquid draws no face against one, while the solid keeps every exposed interface, including submerged sides; `markLightSource` added, tagging each tile with its node's light source so emitting and non-emitting nodes cannot batch together; `drawSolidNode` and `drawNodeboxNode` both draw a node's sub node carve in place of its ordinary geometry, from the live dig, from a carve the server has stored on the node, or (`drawSolidNode` only) from the demo row; the base shape is a plain cube for `drawSolidNode` and the node's own resolved boxes (`getNodeBoxes`, already resolved for its live connections) for `drawNodeboxNode`; a neighbour that is itself carved counts as not solid for both the face mask and the backing faces on the solid path, so no boundary is closed with material a damaged neighbour no longer has; the cut faces clear `MATERIAL_FLAG_CRACK` so they keep the tile's own material rather than a composited crack tile; `markCrackStage` (added) tags a stored carve's own outer faces with `GOANNA_PERSISTENT_CRACK` so they show their own accumulated stage rather than only the node under this player's crosshair; no bevel while the building thread has `g_goanna_plain_solids` set, which only inventory item meshes do |
