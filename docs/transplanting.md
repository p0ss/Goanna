...
| `src/transplant/client/content_mapblock.cpp` | `src/client/content_mapblock.cpp` | Reaches `Client` | Compiles against Goanna's `Client` stand-in; `applyFacesShading` calls are skipped while `g_goanna_no_light` is set; `drawBeveledSolid` added, and `drawSolidNode` routes bevelled nodes to it while `g_goanna_bevel` is above zero; `isFakeLiquid` added, a liquid draws no face against one, and one draws no sideways face against a liquid source at its own level; `markLightSource` added, tagging each tile with its node's light source so emitting and non-emitting nodes cannot batch together |
