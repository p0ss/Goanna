...
| Element | Dark glass |
| --- | --- |
| `background[]`, `background9[]` from the prepend, or in the form with a texture the prepend's backgrounds use | Replaced by a glass pane of the same rectangle. |
...
| `style[]`, `style_type[]` of the form's own | Kept. A button coloured with `bgcolor` becomes a tinted glass button; one with a `bgimg` keeps its art. |
...
| `image[]` behind a slot, framing it, whose texture frames at least two slots in the form | A slot frame: hidden, and the slot draws its own glass tile. |
...
lies behind the slot; it contains the slot and is at most a quarter of a slot
larger on each side; and the same texture frames at least two slots. This
catches Mineclonia's `mcl_formspec_itemslot.png` under every slot and
...
can (the only slot framed with that texture) and anything bigger than a slot.
...
`background[]` or `background9[]` with the form's own texture covers nine
tenths of the form or more, as a Mineclonia book does, the form is drawn
exactly as in the game theme, text colours and all, because its text and
controls were made for that art.
...
- Mineclonia's creative inventory tabs, and the survival inventory tabs a
  mod can add, are the game's own art (`style[...;bgimg=...]` and `image[]`
  of the form's own), so they are kept as light grey tabs above and below
  the dark window. The rule keeps them because their art carries which tab
  is selected.
- Dark and mid tone items stand out less on the glass slot tile than on
  ...
  `glass_style.gd`.
- Mineclonia's empty armour slot outlines are black line art drawn for a
  light grey slot. On the glass tile they are visible but fainter than in
  the game theme.
